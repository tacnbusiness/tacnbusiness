    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <header className="flex items-center justify-between mb-8">
        ...
        <nav>
          <Link href="/add-business">
            <a className="btn-primary mr-4">Add Your Business</a>
          </Link>
          <Link href="/login">
            <a className="btn-outline">Login</a>
          </Link>
        </nav>
      </header>
      ...
      <section className="mb-12">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 gap-6">
          {categories.map(cat => (
            <Link key={cat.slug} href={`/businesses?category=${cat.slug}`}>
              <a className="flex flex-col items-center p-4 border rounded-md hover:shadow-md transition">
      ...
      <section className="mb-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {/* You’ll fetch featured businesses from your database */}
          {/* For now, placeholder cards: */}
          <div className="border p-4 rounded shadow">
            ...
            <a href="https://wa.me/2348012345678" target="_blank" rel="noreferrer" className="text-green-600 font-semibold">Chat on WhatsApp</a>
          </div>
          {/* Repeat similar cards for other featured businesses */}
        </div>
      </section>
      ...
      <section className="mb-12">
        ...
        <div className="flex flex-wrap gap-4">
          {states.map(state => (
            <Link key={state} href={`/businesses?state=${state}`}>
              <a className="px-4 py-2 border rounded hover:bg-blue-600 hover:text-white transition">{state}</a>
            </Link>
          ))}
        </div>
      </section>
      ...
      <section className="text-center mb-12">
        <Link href="/subscribe">
          <a className="btn-primary px-6 py-3">Subscribe to Get Featured</a>
        </Link>
      </section>
